...
function updateContent(unSortedList: list, callback?: () => void) {
  ...
  for (let entry of list) {
    ...
    span = $("<span></span>");
    span.addClass(
      "btn  btn-warning btn-sm glyphicon glyphicon-repeat resetPin"
    );
    span.data("uid", entry.uid);
...
function resetPinButtonClick() {
  ...
  getList((list) => {
    ...
    for (let k in entry) {
      ...
      $("<div/>", resetPinDialogLabel).appendTo(
        "#resetPin_dialog_label_container"
      );
    }
...
function getToken(
  data: string,
  callback: (token: string, salt: string) => void
) {
  $.ajax({
    url: "/getSalt",
    type: "GET",
...
const setCookie = function setCookieWrapper(
  c_name: string,
  value: string,
  exdays?: number
) {
...
function getCookie(c_name: string): string {
  ...
  for (let cookie of cookies) {
    ...
    if (name.trim() === c_name) return unescape(value);
  }
  return "";
}
function matchHn(str: string): boolean {
...
function matchIp(str: string): boolean {
  ...
  return (
    ...
    /(^\s*((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?))\s*$)/.test(
      str
    )
  );
}
