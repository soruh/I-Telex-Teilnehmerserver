...
function connect(onEnd, options) {
    return new Promise((resolve, reject) => {
        ...
        logger.info(misc_js_1.inspect `${colors_js_1.default.FgGreen}trying to connect to: ${colors_js_1.default.FgCyan}${serverkey}${colors_js_1.default.Reset}`);
        ...
        socket.on('end', () => {
            ...
            logger.info(misc_js_1.inspect `${colors_js_1.default.FgYellow}server ${colors_js_1.default.FgCyan}${client.name}${colors_js_1.default.FgYellow} ended!${colors_js_1.default.Reset}`);
            // logger.info(inspect`${colors.FgGreen}deleted connection ${colors.FgCyan+client.name+colors.Reset}`);
            client = null;
            onEnd(client);
        });
        socket.on('timeout', () => {
            logger.warn(misc_js_1.inspect `${colors_js_1.default.FgRed}server: ${colors_js_1.default.FgCyan}${serverkey}${colors_js_1.default.FgRed} timed out${colors_js_1.default.Reset}`);
            // socket.emit("end");
        ...
        socket.on('error', error => {
            if (error["code"] != "ECONNRESET") { //TODO:  alert on ECONNRESET?
                ...
                logger.info(misc_js_1.inspect `${colors_js_1.default.FgRed}server ${colors_js_1.default.FgCyan}${serverkey}${colors_js_1.default.FgRed} could not be reached; errorCounter: ${colors_js_1.default.FgCyan}${misc_js_1.errorCounters[serverkey]}${colors_js_1.default.Reset}`);
            }
            else {
