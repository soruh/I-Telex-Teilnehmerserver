...
import { inspect, normalizeIp, decodeExt, encodeExt } from "../SHARED/misc.js";
...
function encPackage(pkg: Package_decoded): Buffer {
	...
	switch (pkg.type) {
		...
		case 5:
			...
			buffer.writeUIntLE(encodeExt(pkg.data.extension) || 0, 95, 1);
	...
	switch (pkg.type) {
		...
		case 5:
			...
			pkg.data = {
				...
				extension: decodeExt(buffer.readUIntLE(95, 1)),
			};
			if (pkg.data.ipaddress === "0.0.0.0") pkg.data.ipaddress = "";
			break;
		case 6:
