				...
				connection.on('data', function(data) {
					...
					if(data[0] == 0x71&&/[0-9]/.test(String.fromCharCode(data[1]))/*&&(data[data.length-2] == 0x0D&&data[data.length-1] == 0x0A)*/){
						...
						if(res[0].length > 0){
							...
							ITelexCom.handlePacket(ITelexCom.decData(res[0]),cnum,pool,connection,handles); //BINARY
						}
					}
				});
			//});
...
function startQWD(){
	...
	qwd.stderr.on('data',function(data){
		if(config.get("QWD_STDERR_LOG") == ""){
			if(ITelexCom.cv(0)) process.stderr.write(colors.FgRed+'qwd stderr: '+colors.FgWhite+data);
	...
	qwd.stdout.on('data',function(data){
		if(config.get("QWD_STDOUT_LOG") == ""){
			if(ITelexCom.cv(0)) process.stdout.write(colors.FgBlue+'qwd stdout: '+colors.FgWhite+data);
		}else if(config.get("QWD_STDOUT_LOG") == "-"){
		}else{
			try{
