							...
							if(res.pin == pin){
								if(res.typ == 5){
									if(ipaddress != res.ipaddresse || port != res.port){
										ITelexCom.SqlQuery(pool,`UPDATE teilnehmer SET port = '${port}', ipaddresse = '${ipaddress}', changed = '1', moddate = ${Math.floor(Date.now()/1000)} WHERE rufnummer = ${number} OR (Left(name, ${config.get("DynIpUpdateNameDifference")}) = Left('${res.name}', ${config.get("DynIpUpdateNameDifference")}) AND port = '${res.port}' AND pin = '${res.pin}' AND type=5)`,function(result_b){
