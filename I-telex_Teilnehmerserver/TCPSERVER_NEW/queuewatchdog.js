...
handles[8][ITelexCom.states.RESPONDING] = (obj,cnum,dbcon,connection,handles)=>{
	if(ITelexCom.cv(2)) console.log(colors.FgMagenta,connections[cnum].writebuffer,colors.FgWhite);
	...
	if(connections[cnum].writebuffer.length > 0){
		...
		if(b){
			...
			if(ITelexCom.cv(1)) console.log(connections[cnum].writebuffer[0]);
			dbcon.query("DELETE FROM telefonbuch.queue WHERE message="+connections[cnum].writebuffer[0].uid+" AND server="+connections[cnum].servernum+";",function(err,res) {
				if(err){
					...
					if(ITelexCom.cv(1)) console.log(colors.FgGreen+"deleted queue entry "+colors.FgCyan+connections[cnum].writebuffer[0].name+colors.FgGreen+" from queue"+colors.FgWhite);
